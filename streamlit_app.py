...
if not st.session_state.logged_in:
    ...
    with col2:
        if not st.session_state.show_register:
            ...
            new_password = st.text_input("新密码", type="password", placeholder="请输入密码（6-12位）")
        ...
        with col_add:
            with st.expander("➕ 添加新装备"):
                with st.form("add_equip_form"):
                    name = st.text_input("装备名称*")
                    equip_type = st.selectbox("装备类型", ["跑鞋", "运动手表", "腰包", "运动服", "其他"])
                    brand = st.text_input("品牌")
                    price = st.number_input("购买价格(元)", min_value=0.0, step=0.1)
                    buy_date = st.date_input("购买日期", value=datetime.now())
                    mileage = st.number_input("初始里程(km)", min_value=0.0, step=0.1)
                    fit_scene = st.selectbox("适用场景", ["公路", "山地", "越野", "田径场", "通用"])
                    
                    if st.form_submit_button("提交添加", type="primary"):
                        if not name:
                            st.error("装备名称不能为空！")
                        else:
            ...
            with st.expander("✏️ 修改装备"):
                if user_equips.empty:
                    ...
                    with st.form("edit_equip_form"):
                        new_name = st.text_input("装备名称", value=equip_to_edit["name"])
                        new_type = st.selectbox("装备类型", ["跑鞋", "运动手表", "腰包", "运动服", "其他"], 
                                              index=["跑鞋", "运动手表", "腰包", "运动服", "其他"].index(equip_to_edit["type"]))
                        new_brand = st.text_input("品牌", value=equip_to_edit["brand"])
                        new_price = st.number_input("购买价格(元)", value=equip_to_edit["price"])
                        new_buy_date = st.date_input("购买日期", value=datetime.strptime(equip_to_edit["buy_date"], "%Y-%m-%d"))
                        new_mileage = st.number_input("累计里程(km)", value=equip_to_edit["mileage"])
                        new_scene = st.selectbox("适用场景", ["公路", "山地", "越野", "田径场", "通用"],
                                               index=["公路", "山地", "越野", "田径场", "通用"].index(equip_to_edit["fit_scene"]))
                        
                        if st.form_submit_button("提交修改", type="primary"):
        ...
        with col_add_sport:
            with st.expander("➕ 添加运动记录"):
                with st.form("add_sport_form"):
                    sport_date = st.date_input("运动日期", value=datetime.now())
                    if user_equips.empty:
                    ...
                    distance = st.number_input("跑步距离(km)", min_value=0.1, step=0.1)
                    pace = st.text_input("平均配速(min/km)", placeholder="例如: 5:30")
                    heart_rate = st.number_input("平均心率(次/分)", min_value=60, max_value=220, step=1)
                    duration = st.number_input("运动时长(分钟)", min_value=1, step=1)
                    
                    if st.form_submit_button("提交添加", type="primary", disabled=submit_disabled):
                        if not pace:
                            st.error("平均配速不能为空！")
                        else:
