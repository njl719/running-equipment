        ...
        with col2:
            if not user_equips.empty:
                ...
                if st.button("生成磨损曲线", type="primary", use_container_width=True, key="wear_curve_btn") and equip['type'] == "跑鞋":
                    ...
                    fig = px.line(chart_data, x="累计使用里程(km)", y="理论缓震性能(%)",
                                  title=f"{equip['brand']} {equip['name']} - 缓震性能衰减曲线")
                    fig.add_hline(y=60, line_dash="dash", annotation_text="预警阈值60%")
                    fig.add_hline(y=30, line_dash="dash", annotation_text="更换阈值30%")
                    curr_cushion = 100 * np.exp(-0.0015 * equip['mileage'])
                    fig.add_scatter(x=[equip['mileage']], y=[curr_cushion], marker_color="red", size=[12],
                                    text=[f"当前{curr_cushion:.1f}%"], mode="markers+text")
                    st.plotly_chart(fig, use_container_width=True)
                    st.info(f"当前状态：缓震性能剩余 {curr_cushion:.1f}%")
    ...
    with tab4:
        ...
        if not user_sports.empty:
            display_sports = user_sports.merge(
                user_equips[["equip_id", "brand", "name"]], on="equip_id", how="left"
            )
            ...
            with col2:
                ...
                if st.button("生成运动统计图表", type="primary", use_container_width=True, key="sport_chart_btn"):
                    ...
                    fig1 = px.line(daily_data, x="date", y="距离(km)", title="每日跑步距离趋势")
                    st.plotly_chart(fig1, use_container_width=True)
                    fig2 = px.line(daily_data, x="date", y="平均心率(次/分)", title="心率变化趋势")
                    st.plotly_chart(fig2, use_container_width=True)
                    fig3 = px.line(daily_data, x="date", y="平均配速(min/km)", title="配速变化趋势")
